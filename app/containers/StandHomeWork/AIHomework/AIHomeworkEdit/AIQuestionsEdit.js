...
export class AIQuestionsEdit extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  ...
  showChangeScore(questionItem) {
    this.setState({
      ...
      changeScoreWay: [1, 5, 6, 7].includes(questionItem.get('templateType')) ? 2 : 1,
      showChangeScoreModal: true,
    });
  }
  aIChangeQuestion(questionItem) {
    ...
    target = target.set('toBeReplaceId', questionItem.get('id')); // 增加一个字段记录需要替换的目标题目id
    setAIHWParamsItem('AIChangeQuestionTarget', target);
    showChangeItemModal();
  }
  changeScore(value, type, i) {
    const { momeryQuestionItem } = this.state;
    const scoreValue = Math.floor(value) || 1;
    let newMomeryQuestionItem = momeryQuestionItem;
    if (type === 'item') {
      newMomeryQuestionItem = newMomeryQuestionItem.set('score', scoreValue);
    } else if (type === 'each') {
      this.setState({ defaultScore: scoreValue });
      const children = (newMomeryQuestionItem.get('children') || fromJS([])).map((it) => it.set('score', scoreValue));
      newMomeryQuestionItem = newMomeryQuestionItem.set('children', children).set('score', children.count() * scoreValue);
    } else if (type === 'only') {
      const children = (newMomeryQuestionItem.get('children') || fromJS([])).setIn([i, 'score'], scoreValue);
  ...
  render() {
    ...
    const { momeryQuestionItem, toolsPosition, changeScoreWay, defaultScore } = this.state;
              ...
              {item.map((it, i) => {
                ...
                >
                  ...
                  <ToolsWtapper className="toolsWtapper" style={Object.assign({}, toolsPositionStyle)}>
                    ...
                    {isChildBU ? null : <ToolItemBox onClick={() => this.aIChangeQuestion(it)}>智能换题</ToolItemBox>}
                    <ToolItemBox>
                      <ErrorCorrect
      ...
      >
        {changeScoreWay === 1 ? <ScoreBox>
          ...
          {changeScoreWay === 2 ? <ScoreBox>
            <p>子题分数：</p>
            <InputNumber min={1} max={99} step={1} value={defaultScore} onChange={(value) => this.changeScore(value, 'each')}></InputNumber>
          </ScoreBox> :
