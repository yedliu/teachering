 ...
 * UEditor
 * version: ueditor
 * build: Wed Oct 30 2019 16:26:56 GMT+0800 (GMT+08:00)
 */
...
(function () {
    ...
    var Editor = UE.Editor = function (options) {
        ...
        /* 尝试异步加载后台配置 */
        /**
         * 移除掉后台配置加载，不需要这东西，前后端分离的今天这种固定形式的配置反倒恶心人
         */
        // me.loadServerConfig();
...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            // 子添加属性限制放开 start
            var _filterImgAttr = ['src', 'width', 'height', 'floatStyle', 'title', 'border', 'alt', 'hspace', 'vspace'];
            var _property = '';
            Object.keys(ci).forEach(function(key){
                var val = ci[key];
                if (_filterImgAttr.indexOf(key) == -1 && val) {
                    _property += key + '="' + val.toString() + '" ';
                }
            })
            // 子添加属性限制放开 end
...
UE.plugins['font'] = function () {
    var me = this,
        ...
        needCmd = {'underline': 1, 'strikethrough': 1, 'wavy': 1, 'fontborder': 1},
        needSetChild = {
            'forecolor': 'color',
            'backcolor': 'background-color',
            'fontsize': 'font-size',
        ...
        (function (cmd, style) {
            UE.commands[cmd] = {
                execCommand: function (cmdName, value) {
                    value = value || (this.queryCommandState(cmdName) ? 'none'
                        : cmdName == 'underline' ? 'underline'
                        : cmdName == 'fontborder' ? '1px solid #000'
                        : cmdName == 'wavy' ? 'wavy underline'
                        : 'line-through');
                    // console.log('execCommand: ', cmdName, value, this.queryCommandState(cmdName), value);
                    var me = this,
                ...
                queryCommandValue: function (cmdName) {
                    ...
                    //trace:946
                    if (cmdName == 'underline' || cmdName == 'strikethrough' || cmdName == 'wavy') {
                ...
                queryCommandState: function (cmdName) {
                    ...
                    // var gg = 0;
                    if (cmdName == 'fontborder') {
                        res = /1px/.test(val) && /solid/.test(val);
                        // gg = 1
                    } else if (cmdName == 'wavy') {
                        res = /wavy/.test(val) && /underline/.test(val);
                        // gg = 2
                    } else {
                        res =  cmdName == 'underline' ? (/underline/.test(val) && !/wavy/.test(val)) : /line\-through/.test(val);
                        // gg = 3
                    }
                    // console.log('queryCommandState: =========================', cmdName, gg, res, val)
                    return res;
                }
            };
        })(p, fonts[p]);
...
(function () {
    ...
    var iframeUrlMap = {
        ...
        'formulakatex': '~/dialogs/formulakatex/zmFormulaDialog.html',
    };
    //为工具栏添加按钮，以下都是统一的按钮触发命令，所以写在一起
    var btnCmds = ['undo', 'redo', 'formatmatch',
        'bold', 'italic', 'underline', 'fontborder', 'wavy', 'touppercase', 'tolowercase',
    ...
    var dialogBtns = {
        ...
        ok:[/*'attachment', */'anchor', 'link',/* 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage', */ 'formulakatex',
            /*'insertvideo', 'edittip', */'edittable', 'edittd',/* 'scrawl', 'template', 'music', 'background', */'charts'],
    };
                    ...
                    editorui[cmd] = function (editor, iframeUrl, title) {
                        ...
                        if (iframeUrl) {
                            ...
                            if (type == 'ok') {
                                ...
                                if (editor.options.showOptionalInDialog.indexOf(cmd) > -1) {
                                    btns.buttons.unshift({
                                        className:'edui-optionalbutton',
                                        label: editor.getLang('optional')[cmd],
                                        editor:editor,
                                        onclick:function () {
                                            // 不关闭，交给注册的地方去处理
                                            // dialog.close(false);
                                            if (utils.isFunction(dialog.onoptional)) {
                                                dialog.onoptional()
                                            } else {
                                                console.log('定制您自己的功能吧！');
                                            }
                                        }
                                    });
                                }
    ...
    */
    ...
    editorui["wavy"] = function (editor) {
        var name = 'wavy';
        var ui = new editorui.Button({
            className: 'edui-for-' + name,
            title: editor.options.labelMap[name] || editor.getLang("labelMap." + name) || '',
            onclick: function () {
                editor.execCommand('wavy');
            },
            theme: editor.options.theme,
        })
