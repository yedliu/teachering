...
import iconShowEye from './images/icon-openEye.png';
import iconHideEye from './images/icon-shutEye.png';
import makeSelectLogin, {
  makeMobile, makePassword, makeIsLoading, makeShowPassword, makeLoginSuccess,
  makeLoginMissText,
} from './selectors';
import {
  defaultAction, changeMobileAction, changePassWordAction, changeShowPasswordAction,
  // changeOpenOrCloseAlertAction,
} from './actions';
      ...
      <div style={{ width: '100%', height: '100%' }}>
        <LoginWrapper>
          ...
          <div style={{ marginTop: '10px' }} className={'formwrapper'}>
            <div className={'messagetip'}>{this.props.loginSuccess ? <span>登陆成功</span> : this.props.loginMissText}</div>
            <div className={'row'}>
              <input className={'mobileInput'} type="text" placeholder="手机" name="mobile" maxLength="11" onKeyPress={this.nextInput} onInput={this.props.mobileChange} value={this.props.mobile} />
            </div>
            <div className={'row'}>
              <input className={'passwordInput'} style={{ paddingRight: '40px' }} type="password" placeholder="密码" name="password" min="8" maxLength="20" onKeyPress={(e) => { if (e.key === 'Enter') this.props.onLoginClick(); }} onInput={this.props.passwordChange} value={this.props.password} />
              <IconEye changeShow={this.props.showPassword} className={'eye_icon'} onClick={this.isShowPassword}></IconEye>
            </div>
