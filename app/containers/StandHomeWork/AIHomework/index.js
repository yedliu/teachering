...
export class AIHomework extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  ...
  checkTree(checked, event) {
    ...
    // const newCheckedList = event.checkedNodes.filter(item => !item.props.children).map((item) => ({ id: toNumber(item.key), level: item.props.level, name: item.props.title })) || [];
    const checkedTree = this.props.AIHomeworkParams.get('AIknowledgeList');
    const data = event.node.props;
    const item = fromJS({ id: toNumber(data.eventKey), level: data.level, name: data.title });
    let newCheckedList;
    let index = 0;
    let exist = false;
    for (let i = 0; i < checkedTree.count(); i++) {
      const id = checkedTree.getIn([i, 'id']);
      if (+item.get('id') === +id) {
        exist = true;
        break;
      }
    ...
    if (exist) {
      newCheckedList = checkedTree.delete(index);
    } else {
      newCheckedList = checkedTree.push(item);
    }
    setAIHWParamsItem('AIknowledgeList', newCheckedList);
