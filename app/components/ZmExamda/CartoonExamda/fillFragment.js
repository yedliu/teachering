...
class FillFragment extends AbstractFragment {
  constructor(props) {
    ...
    let total = 0;
    try {
      total = content.match(eval("/answerstring/ig")).length;
    } catch (e) {
      console.warn('题目数据异常，id：', question.get('id'), e);
    }
  ...
  render() {
    ...
    const content = question.get('content') || '';
    const showRightAnswer = this.props.showRightAnswer;
    ...
    setTimeout(() => {
      ...
      [...(this.fillBox.getElementsByTagName('zmFill') || [])].forEach((ele, index) => {
        ...
        // 增加是否正确标示
        if (showCorrection) {
          let correct = showRightAnswer || optionList[answerListSelf[index]] == answerList[index];
    ...
    const contentProps = {
      ...
      className: 'examda-fillContainer',
      dangerouslySetInnerHTML: { __html: content }
    };

    return (
      <div {...boxProps} className='examda-fill-box-wrapper'>
        <div className="examda-content">
        ...
        <div className="examda-optionPoll">
          {optionList.map((option, index) => {
            ...
            return <DraggableCore {...optionProps}><span {...childOption}></span></DraggableCore>;
          })}
        </div>
      </div>
    );
