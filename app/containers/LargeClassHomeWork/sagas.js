...
import request, { postjsonoptions, getjsonoptions, putjsonoptions, deletejsonoptions } from 'utils/request';
...
import { changeBtnCanClickAction, changeAlertShowOrHideAction, setAlertStatesAction } from 'containers/LeftNavC/actions';
...
import {
  ...
  changePageState,
  findSystemDict as findSystemDictAction,
} from './actions';
...
export function* getGradeList() {
  // const requestURL = `${Config.trlink}/api/courseSystem/grade`;
  ...
  try {
    ...
    switch (repos.code.toString()) {
      case '0':
        ...
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('gradeList', fromJS(repos.data || [defaultGrade])).set('selectGrade', fromJS(selectedGrade))));
        yield put(getSubjectListAction());
        break;
      default:
        yield put(prviewSelectObj.set('gradeList', fromJS([])));
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('gradeList', fromJS([defaultGrade])).set('selectGrade', fromJS(defaultGrade))));
...
export function* getSudjectList() {
  ...
  try {
    ...
    // console.log(repos, 'getSudjectList');
    switch (repos.code.toString()) {
      case '0':
        const selectSubject = repos.data[0] || defaultSubject;
        yield put(setAIHWParamsItemAction('subject', selectSubject.name));
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('sudjectList', fromJS(repos.data || [defaultSubject])).set('selectSubject', fromJS(selectSubject))));
        yield put(getEditionListAction());
        break;
      default:
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('sudjectList', fromJS([defaultSubject])).set('selectSubject', fromJS(defaultSubject))));
        yield put(prviewSelectObj.set('sudjectList', fromJS([{ id: -1, name: '请选择' }])));
...
export function* getEditionList() {
  const requestURL = `${Config.trlink}/api/lectureCourseContent/edition`;
  const prviewSelectObj = yield select(makePrviewSelectObj());
  // const gradeId = prviewSelectObj.getIn(['selectGrade', 'id']) || -1;
  ...
  try {
    const repos = yield call(request, requestURL, Object.assign({}, getjsonoptions()), {});
    // const repos = yield editionApi.getEdition({ gradeId, subjectId, classTypeCode: 1 });
    // console.log(repos, 'getEditionList');
    switch (repos.code.toString()) {
      case '0': {
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('editionList', fromJS(repos.data || [defaultEditon])).set('selectEdition', fromJS(repos.data[0] || defaultEditon))));
        const searchQuestionParams = yield select(makeSearchQuestionParams());
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('editionList', fromJS(repos.data || []).unshift(fromJS({ id: -1, name: '全部' })))));
        // 获取时间段
        yield put(findSystemDictAction({ groupCode: 'TERM_CODE' }, '时间段'));
        // yield put(getCourseListAction());
        break;
      }
      default:
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('editionList', fromJS([defaultEditon])).set('selectEdition', fromJS(defaultEditon))));
...
export function* getCourseList(item) {
  const prviewSelectObj = yield select(makePrviewSelectObj());
  const courseSystemMap = prviewSelectObj.get('courseSystemMap');
  if (courseSystemMap.get(item.courseContentId)) {
    console.log('课程体系有缓存了', courseSystemMap.toJS());
    return;
  }
  const requestURL = `${Config.trlink}/api/lectureCourseSystem/search`;
  try {
    const repos = yield call(request, requestURL, Object.assign({}, getjsonoptions()), {
      lectureCourseContentId: item.courseContentId
    });
    switch (toString(repos.code)) {
      case '0':
        if (repos.data && repos.data.length > 0) {
          const newMap = courseSystemMap.set(item.courseContentId, fromJS(repos.data));
          yield put(setPreviewSelectObjAction(prviewSelectObj.set('courseSystemMap', newMap)));
        }
        //   const defaultSelect = yield backDefaultKnowledge(repos.data) || selectKnowledge;
        //   selectKnowledge = { id: defaultSelect.id, name: defaultSelect.name, level: defaultSelect.level, path: getTreePath(fromJS(repos.data || []), [0, 0, 0, 0]) };
        //   const path = yield pathFinish(backPath(repos.data || [selectKnowledge]));
        //   // console.log(path, 'path - getCourseList');
        //   const searchQuestionParams = yield select(makeSearchQuestionParams());
        //   yield put(setSearchQuestionParamsAction(searchQuestionParams.set('selectCourseSystemPath', fromJS(path))));
        ...
        //   yield put(setCreateHomeworkStepParamsAction(createHomeworkStepParams.set('homeworkName', `${selectKnowledge.name}--${createHomeworkStepParams.getIn(['homeworkDiff', 'name'])}`)));
        //   yield put(setAIHWParamsAction(AIHomeworkParams
        //     .set('homeworkName', `${selectKnowledge.name}--${AIHomeworkParams.getIn(['homeworkDiff', 'name'])}`)
        //     .set('selectCourseSystem', fromJS({ id: selectKnowledge.id, name: selectKnowledge.name, level: selectKnowledge.level }))));
        // }
        // yield put(setPreviewSelectObjAction(prviewSelectObj.set('treeList', fromJS(repos.data || [])).set('selectTree', fromJS(selectKnowledge)).set('treeDataIsLoading', false)));
...
export function* getStandhomeworkList() {
  const createHomeworkStepParams = yield select(makeCreateHomeworkStepParams());
  const lcsId = createHomeworkStepParams.get('lcsId');
  const lccId = createHomeworkStepParams.get('lccId');
  // console.log('课程体系id', lcsId);
  // console.log('课程内容id', lccId);
  if (!lccId && !lcsId) {
    message.info('请选择课程内容或课程体系');
    return;
  }
  yield put(changePageState('isLoading', true));
  // const requestURL = `http://192.168.25.179:8080/hw-web/api/bigClazzStandardHomework`;
  const requestURL = `${Config.tklink}/api/bigClazzStandardHomework`;
  const prviewSelectObj = yield select(makePrviewSelectObj());
  ...
  if (serachParams.get('scene') > 0) {
    params.applicableScene = serachParams.get('scene');
  }
  // params.edition = prviewSelectObj.getIn(['selectEdition', 'id']);
  // console.log(params, prviewSelectObj.toJS(), 'params - params');
  try {
    const repos = yield call(request, requestURL, Object.assign({}, getjsonoptions()), params);
    // const repos = homeworkList;
    switch (toString(repos.code)) {
      case '1':
        // console.log(repos, 'getStandhomeworkList');
        // const selectTree = repos.data[0] || { id: -1, name: '未找到课程体系' };
        // console.log(selectTree, 'selectTree');
        // yield put(setPreviewSelectObjAction(prviewSelectObj.set('treeList', fromJS(repos.data || [])).set('selectTree', fromJS(Object.assign({}, repos.data[0], { level: 1 }) || { id: -1, name: '未找到课程体系', level: 0 }))));
        // yield put(getStandhomeworkListAction());
        console.log('取消loading');
        yield put(changePageState('isLoading', false));
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('standHomeWorkList', fromJS(repos.data.data || [])).set('paperTotal', repos.data.total || 0)));
        break;
      default:
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('standHomeWorkList', fromJS([]))));
        yield put(changePageState('isLoading', false));
  ...
  try {
    ...
    switch (repos.code.toString()) {
      case '0': {
        ...
        if (repos.data && repos.data.length > 0) {
          selectPhaseSubject = repos.data.find((item) => {
            if (item.phaseId === backPhaseId(selectGrade.get('id')) && item.subjectId === selectSubject.get('id')) {
              return item;
            }
            return null;
          }) || repos.data[0];
        ...
        const searchQuestionParams = yield select(makeSearchQuestionParams());
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('phaseSubjectList', fromJS(repos.data || [selectPhaseSubject]))
...
export function* getKnowledgeList(action) {
  ...
  yield put(setSearchQuestionParamsAction(searchQuestionParams.set('knowledgeListIsLoading', true)));
  const phaseSubjectId = searchQuestionParams.getIn(['selectPhaseSubject', 'id']) || -1;
  const selectSubjectId = searchQuestionParams.getIn(['selectPhaseSubject', 'subjectId']) || -1;
  ...
  try {
    ...
    switch (repos.code.toString()) {
      case '0':
        if (repos.data && repos.data.length > 0) {
          const defaultSelect = backDefaultKnowledge(repos.data) || selectKnowledge;
          selectKnowledge = { id: defaultSelect.id, name: defaultSelect.name, idList: [toString(defaultSelect.id)].concat(backChildren(defaultSelect.children, 'saga')), path: backPath(repos.data || [selectKnowledge]) };
        }
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('knowledgeList', fromJS(repos.data || []))
    ...
    yield put(setSearchQuestionParamsAction(searchQuestionParams.set('knowledgeListIsLoading', false)));
...
export function* getQuestionTypeList() {
  ...
  try {
    const repos = yield queryNode.queryAllQuestionType();
    // call(request, requestURL, Object.assign({}, getjsonoptions()));
    switch (toString(repos.code)) {
      case '0': {
        const res = fromJS([selectType].concat((repos.data || [])));
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('questionTypeList', fromJS(res))));
        break;
      }
      default:
...
export function* getQuestionList() {
  ...
  yield put(setCreateHomeworkStepParamsAction(createHomeworkStepParams.set('questionDataList', fromJS([]))
  ...
  const params = {
    ...
    orderByDirectionStr: `${searchQuestionParamsJS.updatedTime},${searchQuestionParamsJS.quoteCount}`,
  };
  if (toNumber(searchQuestionParamsJS.selectedGrade.id) >= 0) params.gradeId = searchQuestionParamsJS.selectedGrade.id;
  if (toNumber(searchQuestionParamsJS.year.id) >= 0) params.year = searchQuestionParamsJS.year.name;
  if (toNumber(searchQuestionParamsJS.term.id) >= 0) params.termId = searchQuestionParamsJS.term.id;
  if (toNumber(searchQuestionParamsJS.province.id) >= 0) params.provinceId = searchQuestionParamsJS.province.id;
  if (toNumber(searchQuestionParamsJS.city.id) >= 0) params.cityId = searchQuestionParamsJS.city.id;
  if (toNumber(searchQuestionParamsJS.county.id) >= 0) params.countyId = searchQuestionParamsJS.county.id;
  if (toNumber(searchQuestionParamsJS.examType.id) >= 0) params.examTypeId = searchQuestionParamsJS.examType.id;
  if (toNumber(searchQuestionParamsJS.paperType.id) >= 0) params.examPaperTypeId = searchQuestionParamsJS.paperType.id;
  if (toNumber(searchQuestionParamsJS.questionType.id) >= 0) params.typeId = searchQuestionParamsJS.questionType.id;
  if (toNumber(searchQuestionParamsJS.difficulty.id) >= 0) params.difficulty = searchQuestionParamsJS.difficulty.id;
  if (searchQuestionParamsJS.input) params.keyword = searchQuestionParamsJS.input;
      ...
      case '0': {
        const questionDataList = (repos.data.data || []).map((item) => {
          const res = Object.assign({}, item, {
            ...
            optionList: (item.optionList || []).map((iit) => backfromZmStandPrev(iit, 'createHw')),
            answerList: (item.answerList || []).map((iit) => backfromZmStandPrev(iit, 'createHw')),
          ...
          if (item.children && item.children.length > 0) {
            ...
            res.children = item.children.map((it) => {
              return Object.assign({}, it, {
                ...
                optionList: (it.optionList || []).map((iit) => backfromZmStandPrev(iit, 'createHw')),
                answerList: (it.answerList || []).map((iit) => backfromZmStandPrev(iit, 'createHw')),
        ...
        yield put(setCreateHomeworkStepParamsAction(createHomeworkStepParams.set('questionDataList', fromJS(questionDataList))
          .set('questionListLoadingOver', true)
...
export function* saveStandHomework() {
  ...
  // const searchQuestionParams = yield select(makeSearchQuestionParams());
  // const selectCourseSystemPath = searchQuestionParams.get('selectCourseSystemPath').toJS();
  ...
  const homeworkScene = (createHomeworkStepParams.get('homeworkScene') || fromJS({})).toJS();
  ...
  const params = {
    ...
    totalScore: (homeworkSkep.map((it) => it.score) || fromJS([])).reduce((a, b) => a + b),
    questionSource: 2,
    compositionMode: 1 // 目前只有手动作业
  };
  params.lcsId = lcsId; // 课程体系
  params.lccId = lccId; // 课程内容
  params.applicableScene = homeworkScene.id;
  let ajaxWay = postjsonoptions;
  if (paperId > 0) {
    params.id = paperId;
    ajaxWay = putjsonoptions;
  }
  // console.log(previewHomework.toJS(), 'previewHomework');
  console.log('要发送的参数', params);
  yield put(setAlertStatesAction(fromJS({ title: '正在保存作业...' })));
  yield put(changeAlertShowOrHideAction(true));
  try {
    const repos = yield call(request, requestURL, Object.assign({}, ajaxWay(), { body: JSON.stringify(params) }));
    // const repos = { code: '0' };
    switch (toString(repos.code)) {
      case '1':
...
export function* getPreviewHomeworkData() {
  ...
  // const requestURL = `http://192.168.25.179:8080/hw-web/api/bigClazzStandardHomework/${id}`;
  ...
  try {
    ...
    switch (toString(repos.code)) {
      case '1':
        repos.data.homeworkQuestionDTOList = repos.data.bigClazzStandardHomeworkQuestionDTOList;
        repos.data.questionSource = 2;
        repos.data.children = repos.data.homeworkQuestionDTOList.map((item) => {
          ...
          const newItem = Object.assign({}, item, {
            questionOutputDTO: Object.assign({}, questionOutputDTO, {
              ...
              analysis: backfromZmStandPrev(questionOutputDTO.analysis, 'createHw'),
              optionList: (questionOutputDTO.optionList || []).map((it) => backfromZmStandPrev(it, 'createHw')).filter((it) => filterHtmlForm(it)),
              answerList: (questionOutputDTO.answerList || []).map((it) => backfromZmStandPrev(it, 'createHw')),
            }),
          });
          ...
          if (children && children.length > 0) {
            newItem.questionOutputDTO.children = children.map((it) => {
              return Object.assign({}, {
                title: backfromZmStandPrev(it.title, 'createHw'),
                analysis: backfromZmStandPrev(it.analysis, 'createHw'),
                optionList: (it.optionList || []).map((itt) => backfromZmStandPrev(itt, 'createHw')).filter((itt) => filterHtmlForm(itt)),
                answerList: (it.answerList || []).map((itt) => backfromZmStandPrev(itt, 'createHw')),
              });
            });
          }
          return newItem;
        ...
        yield put(setPreviewHomeworkDataListAction(previewHomework.set('homeworkDataList', fromJS(repos.data || {})).set('isOpen', true)));
        break;
      default:
...
export function* editorHomework() {
  const previewHomework = yield select(makePreviewHomework());
  yield put(setPreviewHomeworkDataListAction(previewHomework.set('isOpen', false)));
  const id = previewHomework.get('homeworkId');
  if (id <= 0) {
    message.error('获取试卷失败，请刷新后再次尝试');
    return;
    ...
    switch (toString(repos.code)) {
      case '1': {
        ...
        const homeworkSkep = yield repos.data.children.map((item) => {
          return Object.assign({}, item.questionOutputDTO, { questionSource: item.questionSource, questionId: item.questionId, score: item.score || 3 });
        });
        ...
        const newHomeworkSkep = yield homeworkSkep.map((item) => {
          ...
          const newItem = Object.assign(item, {
            title: backfromZmStandPrev(item.title, 'createHw'),
            analysis: backfromZmStandPrev(item.analysis, 'createHw'),
            optionList: (item.optionList || []).map((it) => backfromZmStandPrev(it, 'createHw')).filter((it) => filterHtmlForm(it)),
            answerList: (item.answerList || []).map((it) => backfromZmStandPrev(it, 'createHw')),
          });
          if (children && children.length > 0) {
            let itemScore = 0;
        ...
        const newCreateHomeworkStepParams = yield createHomeworkStepParams
          ...
          .set('homeworkStep', 2)
          .set('lcsId', repos.data.lcsId)
          .set('lccId', repos.data.lccId)
          // .set('homeworkName', fromJS({ knowledge: homeworkName.slice(0, -4), diff: homeworkName.slice(-2) }))
          .set('homeworkName', homeworkName)
          // eslint-disable-next-line eqeqeq
          .set('homeworkScene', sceneList.find(it => repos.data.applicableScene == it.get('id')));
        // console.log(newCreateHomeworkStepParams.toJS(), 'newCreateHomeworkStepParams');
        yield put(setCreateHomeworkStepParamsAction(newCreateHomeworkStepParams));
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('showCreateHomeworkModal', true)));
        break;
      }
      default:
        message.warning('系统异常导致获取作业预览失败');
  ...
  try {
    ...
    switch (repos.code.toString()) {
      case '0': {
        ...
        const searchQuestionParams = yield select(makeSearchQuestionParams());
        const selectPhaseSubject = searchQuestionParams.get('selectPhaseSubject');
        const newGradeList = homeworkGradeList(allGradeList, selectPhaseSubject);
        yield put(setSearchQuestionParamsAction(searchQuestionParams.set('gradeList', newGradeList)));
        break;
      }
      default:
...
function* deleteHomework() {
  ...
  const requestURL = `${Config.tklink}/api/bigClazzStandardHomework/${id}`;
  try {
    const repos = yield call(request, requestURL, Object.assign({}, deletejsonoptions()));
    switch (repos.code.toString()) {
      case '1':
...
function* getKnowledgeListByCsId() {
  ...
  try {
    const repos = yield call(request, requestURL, Object.assign({}, getjsonoptions()), { csId });
    switch (toString(repos.code)) {
      case '0':
        yield put(setAIHWParamsItemAction('AIknowledgeList', fromJS(repos.data || [])));
...
function* getQuestionType4AiHw() {
  ...
  const params = {
    ...
    knowledgeIdList: AIHomeworkParams.get('AIknowledgeList').map((item) => item.get('id')).toJS(),
    phaseSubjectId,
  ...
  if (AIHomeworkParams.get('termId') > 0) params.termId = AIHomeworkParams.get('termId');
  if (AIHomeworkParams.get('gradeId') > 0) params.gradeId = AIHomeworkParams.get('gradeId');
  try {
    ...
    switch (toString(repos.code)) {
      case '0':
        ...
        const subjectId = searchQuestionParams.getIn(['selectPhaseSubject', 'subjectId']);
        yield put(setAIHWParamsAction(AIHomeworkParams
          .set('questionTypeList', fromJS(data || []).map((item) => {
            const questionCount = item.get('questionAmount') || 0;
            let newItem = item.set('questionCount', questionCount);
            if (subjectId === 1) {
              switch (item.get('name')) {
                case '选择题':
                  newItem = newItem.set('questionAmount', questionCount >= 6 ? 6 : questionCount).set('isActive', true);
                  break;
                case '填空题':
                  newItem = newItem.set('questionAmount', questionCount >= 2 ? 2 : questionCount).set('isActive', true);
                  break;
                case '阅读题':
                  newItem = newItem.set('questionAmount', questionCount >= 1 ? 1 : questionCount).set('isActive', true);
                  break;
                case '写作题':
                  newItem = newItem.set('questionAmount', questionCount >= 1 ? 1 : questionCount).set('isActive', true);
                  break;
                default:
                  newItem = newItem.set('questionAmount', 0).set('isActive', false);
                  break;
              }
            } else {
              switch (item.get('name')) {
                case '选择题':
                  newItem = newItem.set('questionAmount', questionCount >= 10 ? 10 : questionCount).set('isActive', true);
                  break;
                case '填空题':
                  newItem = newItem.set('questionAmount', 0).set('isActive', true);
                  break;
                case '解答题':
                  newItem = newItem.set('questionAmount', 0).set('isActive', true);
                  break;
...
function* getQuestion4AIHW() {
  ...
  const params = {
    ...
    itemDTOList: AIHomeworkParams.get('questionTypeList').filter((item) => item.get('isActive') && item.get('questionAmount') > 0).map((item) => ({ id: item.get('id'), questionAmount: item.get('questionAmount') })).toJS(),
    knowledgeIdList: AIHomeworkParams.get('AIknowledgeList').map((item) => item.get('id')).toJS(),
    phaseSubjectId,
    investigateScope: AIHomeworkParams.get('investigateScope'),
  ...
  try {
    ...
    switch (toString(repos.code)) {
      case '0':
        ...
        yield put(setAIHWParamsAction(AIHomeworkParams
          .set('AIHWQuestionList', fromJS(data)
            .sortBy((item) => item.get('parentTypeId'))
            .map((item) => item.set('showTools', false).set('showAnalysis', false)))
          .set('state', 2).set('isGettingAIHWQuestionList', false)));
        break;
      default:
        break;
    }
...
export function* getChangeItemDataList(action) {
  ...
  const homeworkParams = isCustomHw ? createHomeworkStepParams : AIHomeworkParams;
  const setParams = isCustomHw ? setCreateHomeworkStepParamsAction : setAIHWParamsAction;
  const questionTarget = homeworkParams.get('AIChangeQuestionTarget').toJS();
  if (!(questionTarget.id > 0)) return;
  const params = {
    difficultyList: backDifficutyType(questionTarget.difficulty || AIHomeworkParams.get('difficulty')),
    itemDTOList: [{ id: questionTarget.parentTypeId, questionAmount: 11 }],
    knowledgeIdList: questionTarget.knowledgeIdList,
    phaseSubjectId: (phaseSubjectList.find((item) => item.get('subjectId') === questionTarget.subjectId) || fromJS({ id: null })).get('id'),
    investigateScope: AIHomeworkParams.get('investigateScope'),
    templateTypes: '1,2,3,4',
  };
  ...
  try {
    ...
    const data = fromJS(repos.data || []).filter((item) => item.get('id') !== questionTarget.id).slice(0, 10);
    switch (toString(repos.code)) {
      case '0':
        yield put(setParams(homeworkParams
          .set('AIChangeQuestionList', fromJS(data || [])
            .map((item) => item.set('showTools', false).set('showAnalysis', true)))
...
function* findSystemDict(item) {
  const param = item.param;
  const prviewSelectObj = yield select(makePrviewSelectObj());
  // const requestURL = `${Config.trlink}/api/dict/findSystemDictByGroupCode`;
  try {
    // const repos = yield call(request, requestURL, Object.assign({}, getjsonoptions()), { ...param });
    const repos = yield dictApi.findSystemDictByGroupCode(param.groupCode);
    switch (toString(repos.code)) {
      case '0':
        const values = Object.values(param);
        if (values.indexOf('LECTURE_COURSE_TYPE') > -1) {
          yield put(setPreviewSelectObjAction(prviewSelectObj.set('courseTypeList', fromJS(repos.data || [])).set('treeDataIsLoading', false)));
        } else if (values.indexOf('TERM_CODE') > -1) {
          yield put(setPreviewSelectObjAction(prviewSelectObj.set('termList', fromJS(repos.data || []))));
          yield put(findSystemDictAction({ groupCode: 'LECTURE_COURSE_TYPE' }, '课程类型'));
        }
        break;
      default:
        break;
    ...
    console.log(err, 'findSystemDict');
  }
}

function* getCourseContent(item) {
  const prviewSelectObj = yield select(makePrviewSelectObj());
  const courseContentMap = prviewSelectObj.get('courseContentMap');
  // 如果map里面有 说明有缓存
  if (courseContentMap.get(`term${item.term}course${item.course}`)) {
    console.log('课程内容有缓存了', courseContentMap.toJS());
    return;
  }
  const gradeId = prviewSelectObj.getIn(['selectGrade', 'id']) || -1;
  const subjectId = prviewSelectObj.getIn(['selectSubject', 'id']) || -1;
  const editionId = prviewSelectObj.getIn(['selectEdition', 'id']) || -1;
  const requestURL = `${Config.trlink}/api/lectureCourseContent/find`;
  const param = {
    termId: item.term,
  ...
  try {
    ...
    switch (toString(repos.code)) {
      case '0':
        const newMap = courseContentMap.set(`term${item.term}course${item.course}`, fromJS(repos.data));
        yield put(setPreviewSelectObjAction(prviewSelectObj.set('courseContentMap', newMap)));
        break;
      default:
        message.error(repos.message || '获取失败');
        break;
    }
  } catch (err) {
    console.log(err, 'getCourseContent');
  }
}
...
export function* getKnowledgeListSaga() {
  const watcher = yield takeLatest(GET_KNOWLEDEGE_LIST_ACTION, getKnowledgeList);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getQuestionTypeListSaga() {
  const watcher = yield takeLatest(GET_QUESTION_TYPE_LIST_ACTION, getQuestionTypeList);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
...
export function* saveStandHomeworkSaga() {
  const watcher = yield takeLatest(SAVE_STAND_HOMEWORK_ACTION, saveStandHomework);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getStandhomeworkListSaga() {
  const watcher = yield takeLatest(GET_STAND_HOMEWORK_LIST_ACTION, getStandhomeworkList);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getPreviewHomeworkDataSaga() {
  const watcher = yield takeLatest(GET_PREVIEW_HOMEWORK_DATA_LIST_ACTION, getPreviewHomeworkData);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
...
export function* getKnowledgeListByCsIdSaga() {
  const watcher = yield takeLatest(GET_KNOWLEDGELIST_BY_CSID_ACTION, getKnowledgeListByCsId);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getQuestionType4AiHwSaga() {
  const watcher = yield takeLatest(GET_QUESTION_TYPE_FOR_AIHW_ACTION, getQuestionType4AiHw);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getQuestion4AIHWSaga() {
  const watcher = yield takeLatest(GET_QUESTION_FOR_AIHW_ACTION, getQuestion4AIHW);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
...
export function* getChangeItemDataListSaga() {
  const watcher = yield takeLatest(GET_CHANGE_ITEMDATALIST_ACTION, getChangeItemDataList);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* findSystemDictSaga() {
  const watcher = yield takeLatest(FIND_SYSTEM_DICT, findSystemDict);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
export function* getCourseContentSaga() {
  const watcher = yield takeLatest(GET_COURSE_CONTENT, getCourseContent);
  yield take(LOCATION_CHANGE);
  yield cancel(watcher);
}
