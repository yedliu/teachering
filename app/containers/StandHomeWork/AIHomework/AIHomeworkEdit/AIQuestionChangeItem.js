...
export class ReplaceQuestionModal extends React.PureComponent {
  // eslint-disable-line react/prefer-stateless-function
  constructor(props) {
    super(props);
    this.state = {
      selectIndex: 1,
      showOriginWhenEmpty: false
    };
  }
  componentDidMount() {
  ...
  render() {
    ...
    const { selectIndex, showOriginWhenEmpty } = this.state;
    const AIChangeQuestionTarget =
      AIHomeworkParams.get('AIChangeQuestionTarget') || fromJS({});
    const AIChangeQuestionList =
      AIHomeworkParams.get('AIChangeQuestionList') || fromJS([]);
    ...
    console.log(AIChangeQuestionTarget.toJS(), '原题');
    return (
      <Masking onClick={() => loading && close()}>
          ...
          {isLoading ? (
            ...
            <Questions>
              <Header>
                  ...
                  return (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (index === 0 && AIChangeQuestionList.count() === 0) {
                          this.setState({ showOriginWhenEmpty: true });
                        }
                      }}
                    >
                      {index === 0 ? (
                        <span style={{ fontSize: 10 }}>原题</span>
                      ) : (
                        index
                      )}
                    </Button>
                ...
                <div className="switch">
                  <Button
                    ...
                    icon="sync"
                    onClick={() => {
                      this.setState({ selectIndex: 1, showOriginWhenEmpty: false });
              ...
              <FlexColumn style={{ padding: 10, overflowY: 'auto', flex: 1 }}>
                {AIChangeQuestionList.count() > 0 || showOriginWhenEmpty ? (
                  <AIQuestionItemEdit
              ...
              <Bottom>
                ...
                <Button
                  disabled={
                    selectIndex === 0 || AIChangeQuestionList.count() <= 0
                  }
                  type="primary"
                  size="large"
                  onClick={() => choose(questionList.get(selectIndex))}
                >
                  替换
                </Button>
              </Bottom>
